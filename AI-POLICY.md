...
- **Dependências inexistentes ou maliciosas** — não adicione uma crate só porque
...
- Seguir as [regras de código](CONTRIBUTING.md#regras-de-código) e a arquitetura
  do projeto.
- Passar na verificação do CI (`cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -W clippy::pedantic -D warnings`, `cargo test --workspace`).
