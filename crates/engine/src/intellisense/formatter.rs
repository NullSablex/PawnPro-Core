...
pub(super) fn split_braces_to_own_lines(text: &str) -> String {
    ...
    // Diretiva continuada com `\` no fim da linha: a próxima linha também é dela.
    let mut directive_continues = false;
    // Comentário de bloco que atravessa linhas: chaves ali dentro são texto.
    let mut in_block_comment = false;
    for raw in text.lines() {
        // Diretiva do preprocessador — inclusive as linhas de continuação de uma
        // macro — sai inteira. Chaves ali são o texto da macro: separá-las mudava
        // o que ela expande, e deixava o `\` da continuação sozinho numa linha.
        // Um `#` dentro de comentário de bloco não é diretiva.
        if !str_continues
            && !in_block_comment
            && (directive_continues || raw.trim_start().starts_with('#'))
        {
            directive_continues = raw.trim_end().ends_with('\\');
...
/// Abre e fecha o marcador que ocupa o lugar de um literal ou comentário
/// durante a formatação. Caracteres de uso privado: nunca aparecem em código
/// Pawn, e para o formatador não são espaço, operador nem palavra — atravessam
/// as etapas intactos.
const MASK_OPEN: char = '\u{E000}';
const MASK_CLOSE: char = '\u{E001}';

/// Onde uma linha termina, para a seguinte saber onde começa.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub(super) enum Carry {
    /// Em código.
    #[default]
    Code,
    /// Dentro de um comentário de bloco que ainda não fechou.
    BlockComment,
    /// Dentro de uma string continuada com `\` no fim da linha.
    String,
}

/// Troca cada literal (string, caractere) e cada comentário da linha por um
/// marcador, e devolve os originais.
///
/// `carry` diz onde a linha começa; o terceiro valor, onde ela termina. O
/// conteúdo mascarado não passa por nenhuma etapa da formatação — cada etapa
/// tentando respeitar strings por conta própria falhava de um jeito: uma não
/// sabia de `\"`, outra trocava `do(` por `do (` dentro das aspas.
fn mask_literals(line: &str, carry: Carry) -> (String, Vec<String>, Carry) {
    let chars: Vec<char> = line.chars().collect();
    let mut masked = String::with_capacity(line.len());
    let mut pieces = Vec::new();
    // Onde começou o comentário de bloco em curso nesta linha.
    let mut block_start = (carry == Carry::BlockComment).then_some(0);
    // Onde começou o literal em curso, e a aspa que o fecha.
    let mut literal: Option<(usize, char)> = (carry == Carry::String).then_some((0, '"'));
    let mut i = 0;
    ...
    while i < chars.len() {
        if let Some(start) = block_start {
            if chars[i] == '*' && chars.get(i + 1) == Some(&'/') {
                i += 2;
                push_masked(&mut masked, &mut pieces, &chars[start..i]);
                block_start = None;
            } else {
                i += 1;
            }
        ...
        if let Some((start, quote)) = literal {
            match chars[i] {
                '\\' => i += 2,
                c if c == quote => {
                    i += 1;
        ...
        match (c, chars.get(i + 1)) {
            ('/', Some('/')) => {
                push_masked(&mut masked, &mut pieces, &chars[i..]);
                i = chars.len();
                continue;
            }
            ('/', Some('*')) => {
    ...
    if let Some(start) = block_start {
        push_masked(&mut masked, &mut pieces, &chars[start..end]);
        return (masked, pieces, Carry::BlockComment);
    }
    if let Some((start, quote)) = literal {
        push_masked(&mut masked, &mut pieces, &chars[start..end]);
        // Só a string continua na linha seguinte, e só com `\` no fim.
        let continues = quote == '"' && line.trim_end().ends_with('\\');
        let carry = if continues {
            Carry::String
        } else {
            Carry::Code
        };
        return (masked, pieces, carry);
    }
    (masked, pieces, Carry::Code)
}

fn push_masked(masked: &mut String, pieces: &mut Vec<String>, text: &[char]) {
    masked.push(MASK_OPEN);
    masked.push_str(&pieces.len().to_string());
    masked.push(MASK_CLOSE);
    pieces.push(text.iter().collect());
}

/// Põe de volta, no lugar de cada marcador, o texto original.
fn unmask(text: &str, comments: &[String]) -> String {
    let mut out = String::with_capacity(text.len());
    let mut rest = text;
    while let Some(open) = rest.find(MASK_OPEN) {
        out.push_str(&rest[..open]);
        let after = &rest[open + MASK_OPEN.len_utf8()..];
        let Some(close) = after.find(MASK_CLOSE) else {
            out.push_str(&rest[open..]);
            return out;
        };
        if let Some(comment) = after[..close]
            .parse::<usize>()
            .ok()
            .and_then(|n| comments.get(n))
        {
            out.push_str(comment);
        }
        rest = &after[close + MASK_CLOSE.len_utf8()..];
    }
    out.push_str(rest);
    out
}
...
///
/// Comentário, string e caractere não são código: saem exatamente como
/// estavam. Antes só a linha que *começava* com comentário era poupada — o
/// `/* ... */` depois do código virava `* /`, que não fecha nada, e o código
/// seguinte sumia para o compilador; o conteúdo de strings continuadas ganhava
/// espaços em volta de `=`, mudando SQL e texto que o jogador vê. `carry` diz
/// onde a linha começa; o segundo valor, onde ela termina.
pub(super) fn format_line(trimmed: &str, space_ops: bool, carry: Carry) -> (String, Carry) {
    let (masked, pieces, next) = mask_literals(trimmed, carry);

    // Diretivas do preprocessador têm sintaxe própria (`<...>` de include, `%0`
    // de macro, continuação com `\`) e NÃO devem passar pelo espaçamento de
    // operadores — senão `#include <a>` viraria `#include < a >`. Linha que só
    // tem literal ou comentário também sai como está. O `*` no começo é a linha
    // de dentro de um bloco que começou antes do trecho formatado.
    let only_masked = masked
        .chars()
        .all(|c| c.is_whitespace() || c == MASK_OPEN || c == MASK_CLOSE || c.is_ascii_digit())
        && !pieces.is_empty();
    if only_masked || trimmed.starts_with('*') || trimmed.starts_with('#') {
        return (trimmed.to_string(), next);
    }

    let s = if space_ops {
        format_operators(&masked)
    } else {
        masked
    };
    let s = format_keyword_spacing(&s);
    (unmask(&collapse_spaces(&s), &pieces), next)
}
