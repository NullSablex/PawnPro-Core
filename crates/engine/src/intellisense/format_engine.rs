...
use super::formatter::{Carry, format_line, split_braces_to_own_lines};
...
pub fn format_with_base(text: &str, style: FormatStyle, base_level: u32) -> String {
    ...
    // Um comentário de bloco ou uma string continuada podem abrir numa linha e
    // fechar em outra: o formatador precisa saber disso ao chegar nas do meio.
    let mut carry = Carry::Code;
    // Macro continuada com `\`: as linhas seguintes são corpo da diretiva, e o
    // espaçamento de operadores transformaria `%0` em `% 0`.
    let mut in_directive = false;
    for info in &line_info {
        if info.is_blank && carry != Carry::String {
            in_directive = false;
        ...
        // Linha que começa dentro de uma string: o recuo dela é conteúdo da
        // string, então sai exatamente como estava — nem recuo novo, nem nada.
        if carry == Carry::String {
            carry = format_line(&info.trimmed, style.space_around_operators, carry).1;
            out.push(info.raw.clone());
            continue;
        }
        let continues_directive = in_directive;
        in_directive =
            (continues_directive || info.trimmed.starts_with('#')) && info.trimmed.ends_with('\\');
        let content = if continues_directive {
            info.trimmed.clone()
        } else {
            let (content, next) = format_line(&info.trimmed, style.space_around_operators, carry);
            carry = next;
            content
        };
...
struct LineInfo {
    trimmed: String,
    /// A linha como veio, para a que continua uma string sair intacta.
    raw: String,
    level: u32,
    is_blank: bool,
}
...
mod tests {
    ...
    // Regressão: string continuada com `\` no fim da linha. A partir da segunda
    // linha o conteúdo era formatado como código — `='%i'` virava ` = '%i'` e
    // `do(a)` virava `do (a)`, mudando o SQL e o texto que o jogador vê.
    #[test]
    fn a_continued_string_is_untouched() {
        let src = "main()\n{\nformat(q, sizeof q, \"UPDATE t SET \\\n`a`='%i', do(a) \\\n`b`='%i'\");\n}\n";
        let out = fmt(src, Preset::Allman);
        assert!(out.contains("\n`a`='%i', do(a) \\\n"), "saída: {out:?}");
        assert!(out.contains("\n`b`='%i'\");"), "saída: {out:?}");
    }

    // Espaços dentro de string são conteúdo — inclusive depois de um `\"`, que
    // enganava o colapso de espaços. E `' '` é o caractere espaço.
    #[test]
    fn spaces_inside_literals_are_kept() {
        let src = "main()\n{\nprint(\"a\\\"    b do(x)\");\nc = ' ';\n}\n";
        let out = fmt(src, Preset::Allman);
        assert!(out.contains("\"a\\\"    b do(x)\""), "saída: {out:?}");
        assert!(out.contains("c = ' ';"), "saída: {out:?}");
    }

    // Regressão: chaves dentro de comentário de bloco — numa linha ou no meio de
    // um bloco de várias — são texto do comentário, não blocos de código. Os
    // includes costumam trazer assinaturas e trechos comentados assim.
    #[test]
    fn braces_inside_block_comments_stay_put() {
        let one_line = "/* native sscanf(const data[], {Float, _}:...); */\n";
        assert_eq!(fmt(one_line, Preset::Allman), one_line);

        let multi = "/*\nnew a[2] = {-1, 0};\n*/\n";
        let out = fmt(multi, Preset::Allman);
        assert!(out.contains("new a[2] = {-1, 0};"), "saída: {out:?}");
    }

    // Regressão: chaves dentro de `#define` são o texto da macro. Levá-las para
    // linhas próprias mudava o que ela expande — e tudo o que depende dela.
    #[test]
    fn braces_inside_a_define_stay_put() {
        let src = "#define StringTag {StringTags}\n";
        assert_eq!(fmt(src, Preset::Allman), src);
    }

    // Macro de várias linhas, continuada com `\`: o corpo inteiro pertence à
    // diretiva, inclusive `{`, `}` e os `%0` dos parâmetros — que o espaçamento
    // de operadores transformaria em `% 0`.
    #[test]
    fn a_continued_macro_body_is_untouched() {
        let src = "#define LOOP(%0) \\\n\tfor (new i; i < %0; i++) { \\\n\t\tf(i); \\\n\t}\n";
        let out = fmt(src, Preset::Allman);
        assert!(
            out.contains("for (new i; i < %0; i++) { \\"),
            "saída: {out:?}"
        );
        assert!(out.contains("f(i); \\"), "saída: {out:?}");
        assert!(!out.contains("% 0"), "saída: {out:?}");
    }

    // Regressão: comentário de bloco depois de código. O conteúdo passava pelo
    // espaçamento de operadores — `*/` virava `* /`, que não fecha mais nada, e
    // o código seguinte sumia para o compilador.
    #[test]
    fn trailing_block_comment_is_untouched() {
        let src = "enum e\n{\n\ta, /* file-mode */\n}\n";
        let out = fmt(src, Preset::Allman);
        assert!(out.contains("/* file-mode */"), "saída: {out:?}");
    }

    // O mesmo com comentário de linha: o texto depois de `//` não é código.
    #[test]
    fn trailing_line_comment_is_untouched() {
        let src = "main()\n{\nx = 1; // a=b-c\n}\n";
        let out = fmt(src, Preset::Allman);
        assert!(out.contains("x = 1; // a=b-c"), "saída: {out:?}");
    }

    // Uma linha no meio de um comentário de bloco é comentário, mesmo sem o `*`
    // que costuma abrir as linhas de dentro.
    #[test]
    fn block_comment_continuation_is_untouched() {
        let src = "main()\n{\nx = 1; /* começa\na=b-c\n*/\n}\n";
        let out = fmt(src, Preset::Allman);
        assert!(out.contains("a=b-c"), "saída: {out:?}");
    }

    // O trecho real que oscilava (`file.inc` do SA-MP): o comentário sai intacto
    // e a segunda passada não muda nada.
    #[test]
    fn samp_file_inc_enum_is_stable() {
        let src = "enum filemode\r\n    {\r\n    io_read,            /* file must exist */\r\n    \
                   io_append,          /* appends to file (write-only) */\r\n    }\r\n\r\n\
                   enum seek_whence\r\n    {\r\n    seek_start,\r\n    }\r\n";
        let once = fmt(src, Preset::Allman);
        assert!(
            once.contains("/* appends to file (write-only) */"),
            "saída: {once:?}"
        );
        let twice = format(&once, FormatStyle::from_preset(Preset::Allman));
    ...
    // Regressão: `enum` com a chave na linha de baixo, recuada à mão — o
    // `file.inc` do SA-MP. A primeira passada e a segunda recuavam o `}` de
    // jeitos diferentes, e o arquivo mudava a cada salvamento. CRLF porque o
    // arquivo real usa, e a quebra de linha entra na conta do recuo.
    #[test]
    fn enum_with_indented_brace_is_idempotent() {
        let lf = "enum filemode\n    {\n    io_read,\n    io_write,\n    }\n";
        for src in [lf.to_string(), lf.replace('\n', "\r\n")] {
            for preset in [Preset::Allman, Preset::Knr, Preset::Compact] {
                let once = fmt(&src, preset);
                let twice = format(&once, FormatStyle::from_preset(preset));
                assert_eq!(once, twice, "preset {preset:?}, fonte {src:?}");
            }
        }
    }
}
