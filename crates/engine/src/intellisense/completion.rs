...
static RX_NEW: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(
        r"(?:^|;|\{)\s*(?:new|static)\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?([A-Za-z_][A-Za-z0-9_]*)",
    )
    .unwrap()
});
...
pub fn get_completions(
    ...
    // O resto da unidade de compilação — o `.inc` irmão, incluído pelo mesmo
    // programa —, do cache: nenhum arquivo é relido a cada tecla.
    let open = state.open_paths();
    let mut unit: Vec<_> = state.unit_files(&file_path, &open).into_iter().collect();
    unit.sort();
    for path in &unit {
        let Some(idents) = state.idents_of(path, &open) else {
            continue;
        };
        for sym in idents.symbols() {
            if !seen.insert(sym.name.clone()) {
                continue;
            }
            let rank = if sym.deprecated {
