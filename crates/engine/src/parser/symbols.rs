...
static RX_NATIVE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*(?:forward\s+)?native\s+(?:[A-Za-z_][A-Za-z0-9_]*::)*(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)").unwrap()
});

static RX_FORWARD: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(
        r"^\s*forward\s+(?:[A-Za-z_][A-Za-z0-9_]*::)*(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)",
    )
    .unwrap()
});
...
static RX_PUBLIC_STOCK: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*(public|stock)(?:\s+(?:public|stock))?\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*((?:[A-Za-z_][A-Za-z0-9_]*::)*)(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)").unwrap()
});
...
static RX_STATIC_STOCK_FUNC: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*static\s+stock\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)").unwrap()
});
...
static RX_STATIC_CONST: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*static\s+const\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\b")
        .unwrap()
});

// stock const [Tag:]Name — constante exportada
static RX_STOCK_CONST: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*stock\s+const\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\b")
        .unwrap()
});
...
static RX_FLOAT_BOOL_FUNC: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*(float|bool)\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)").unwrap()
});
...
static RX_PLAIN_FUNC: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*((?:[A-Za-z_][A-Za-z0-9_]*::)*)(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)")
        .unwrap()
});

// #define Name — captura o nome e tudo que vem depois (corpo da macro)
static RX_DEFINE: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*#\s*define\s+([A-Za-z_][A-Za-z0-9_]*)\b(.*)$").unwrap()
});
...
static RX_MACRO_PREFIX: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*#\s*define\s+([A-Za-z_][A-Za-z0-9_]*)(?:::?)\s*%\d").unwrap()
});
...
static RX_NAMESPACE_ALIAS: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*#\s*define\s+([A-Za-z_][A-Za-z0-9_]*)::\s+([A-Za-z_][A-Za-z0-9_]*)").unwrap()
});

// Detecta abertura de alias de namespace com continuação de linha:
// `#define NS:: \` — o alias está na próxima linha
static RX_NAMESPACE_ALIAS_CONT: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
...
static RX_FUNC_OPEN: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*((?:(?:public|stock|static|native|forward)\s+)*)?(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*((?:[A-Za-z_][A-Za-z0-9_]*::)*)(?:[A-Za-z_][A-Za-z0-9_]*:)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)$").unwrap()
});

// enum [Tag:] [Name] [(opts)] [{] — grupo 1: nome opcional do enum
static RX_ENUM: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*enum\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?([A-Za-z_][A-Za-z0-9_]*)").unwrap()
});

// Membro de enum: [Tag:]Name (dentro do corpo)
static RX_ENUM_MEMBER: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
...
static RX_STATIC_VAR: std::sync::LazyLock<Regex> = std::sync::LazyLock::new(|| {
    Regex::new(r"^\s*static\s+(?:[A-Za-z_][A-Za-z0-9_]*:)?([A-Za-z_][A-Za-z0-9_]*)\s*(?:[=\[;])")
        .unwrap()
});
...
/// corpo abra adiante. Devolve a assinatura enquanto ela segue em aberto.
fn continue_multiline_func(
    line: &str,
    mut mf: MultilineFunc,
    result: &mut ParsedFile,
    pending_plain: &mut Option<PendingPlain>,
) -> Option<MultilineFunc> {
    ...
    let (pidx, pcol, pname, pparams, pkind, pdep, pdoc) = mf;
...
/// Um `#include`/`#tryinclude` numa linha já sem comentários.
fn include_directive(line: &str, raw_line: &str, line_idx: usize) -> Option<IncludeDirective> {
    let cap = RX_INCLUDE.captures(line)?;
    let is_try = cap.get(1).is_some();
    // A expressão só casa com `<...>` (grupo 2) ou `"..."` (grupo 3).
    let (token, is_angle) = match (cap.get(2), cap.get(3)) {
        (Some(m), _) => (m.as_str().trim().to_string(), true),
        (None, Some(m)) => (m.as_str().trim().to_string(), false),
...
impl ParserState {
    ...
    fn process_line(&mut self, raw_line: &str, line_idx: usize, raw_lines: &[&str]) {
        ...
        if let Some(mf) = self.multiline_func.take() {
            self.multiline_func =
                continue_multiline_func(&line, mf, &mut self.result, &mut self.pending_plain);
    ...
    fn process_top_level(
        ...
        if RX_INCLUDE.is_match(line) {
            if let Some(directive) = include_directive(line, raw_line, line_idx) {
...
mod tests {
    ...
    #[test]
    fn a_block_right_above_the_declaration_is_its_doc() {
        let src = "/**\n * Bane alguém.\n */\nstock Ban() { return 1; }\n";
        assert_eq!(
    ...
    #[test]
    fn contiguous_comment_lines_are_doc() {
    ...
    #[test]
    fn only_the_adjacent_block_counts() {
    ...
    #[test]
    fn a_blank_line_separates_doc_from_declaration() {
    ...
    #[test]
    fn code_between_comment_and_declaration_breaks_the_link() {
        let src =
            "// Doc da primeira.\nstock Primeira() { return 1; }\nstock Segunda() { return 1; }\n";
        assert_eq!(doc_de(src, "Segunda"), None);
    }
    ...
    #[test]
    fn rulers_and_section_headers_do_not_leak_into_the_symbol() {
    ...
    #[test]
    fn a_header_right_above_the_declaration_is_still_read() {
    ...
    #[test]
    fn a_pragma_between_doc_and_declaration_does_not_break() {
        let src = "/**\n * Bane alguém.\n */\n#pragma deprecated Use BanPlayerFor\nstock Ban() { return 1; }\n";
        let f = parse_file(src);
    ...
    #[test]
    fn a_block_end_without_opening_is_not_doc() {
    ...
    #[test]
    fn a_declaration_on_the_first_line_has_no_doc() {
