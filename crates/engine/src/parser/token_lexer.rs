...
impl<'s> Lexer<'s> {
    ...
    fn step(&mut self) {
        ...
        // O compilador aceita recuo antes do `#`: o que decide é ser o primeiro
        // caractere da linha, não estar na coluna 0.
        ...
        if ch == b'#' && first_on_line {
            self.lex_directive(tok_line, tok_col, tok_indent);
            return;
        }
    ...
    fn lex_directive(&mut self, line: u32, col: u32, indent: u32) {
        self.advance(); // '#'
        self.push(TokenKind::Hash, "#", line, col, indent);
...
mod tests {
    ...
    /// Os tokens sem posição, para comparar a mesma linha com e sem recuo.
    ...
    #[test]
    fn an_indented_directive_is_a_directive() {
        // O compilador aceita recuo antes do `#`: a linha recuada tem de dar os
        // mesmos tokens que a sem recuo.
        for src in ["#include <a_samp>", "#define MAX 10", "#pragma unused x"] {
            assert_eq!(shape(&format!("    {src}")), shape(src), "{src}");
            assert_eq!(shape(&format!("\t{src}")), shape(src), "{src}");
        }
    }

    #[test]
    fn an_indented_hash_keeps_its_column() {
        let ts = tokenize("    #if defined X");
        assert_eq!(ts.tokens[0].kind, TokenKind::Hash);
        assert_eq!(ts.tokens[0].col, 4);
        assert_eq!(ts.tokens[1].kind, TokenKind::Directive);
        assert_eq!(ts.tokens[1].value, "if");
    }

    #[test]
    fn a_tab_between_hash_and_name_is_still_a_directive() {
        assert_eq!(shape("#\tinclude <a>"), shape("#include <a>"));
    }

    #[test]
    fn a_hash_mid_line_is_not_a_directive() {
        assert!(!kinds("x = 1 # 2;").contains(&TokenKind::Directive));
    }

    #[test]
    fn a_hash_after_a_continuation_is_not_a_directive() {
        // Na continuação de uma macro, a linha seguinte ainda é a mesma diretiva.
        let k = kinds("#define A \\\n    #B");
        assert_eq!(k.iter().filter(|k| **k == TokenKind::Directive).count(), 1);
    }
}
