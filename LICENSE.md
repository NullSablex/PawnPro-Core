# Licença PawnPro-Core (PawnPro-Core License)
**Versão 1.0 - 2026**

Copyright (c) 2025-2026 NullSablex  
Os direitos não concedidos expressamente nesta licença ficam reservados ao autor.

---

## 📋 RESUMO SIMPLES

✅ **PODE:**
- Usar gratuitamente (pessoal e comercial)
- Ver e estudar o código
- Modificar e redistribuir gratuitamente, com atribuição
- Contribuir com melhorias

❌ **NÃO PODE:**
- Vender ou cobrar pelo software
- Redistribuir comercialmente
- Criar versões pagas/premium
- Remover créditos do autor

---

## 1. DEFINIÇÕES

**"Software"** refere-se ao PawnPro-Core — o núcleo (`pawnpro-core`), a engine (`pawnpro-engine`), o adaptador de depuração (`pawnpro-dap-adapter`), o protocolo de depuração (`pawnpro-dbg-protocol`) e o plugin de depuração (`pawnpro-debug-plugin`) — e todo seu código-fonte, documentação e arquivos relacionados.

**"Você"** refere-se à pessoa física ou jurídica exercendo as permissões concedidas por esta licença.

**"Uso Comercial"** significa usar o Software em atividades que geram receita, lucro ou compensação financeira.

**"Venda"** significa cobrar dinheiro, criptomoedas ou qualquer compensação pela distribuição do Software ou suas versões modificadas.

**"Modificação"** significa qualquer alteração, adição, remoção ou derivação do código-fonte original.

---

## 2. CONCESSÃO DE DIREITOS

Sujeito aos termos e condições desta licença, o autor concede a você uma licença mundial, livre de royalties, não exclusiva e irrevogável — salvo a rescisão prevista na seção 9 — para:

### 2.1 USO
- Instalar e executar o Software para qualquer finalidade, incluindo uso comercial, observadas as restrições da seção 3
- Utilizar o Software em projetos pessoais ou corporativos
- Integrar o Software no seu fluxo de trabalho de desenvolvimento

### 2.2 VISUALIZAÇÃO E ESTUDO
- Acessar e estudar o código-fonte
- Aprender com a implementação
- Analisar a arquitetura e design

### 2.3 MODIFICAÇÃO
- Criar modificações para uso próprio
- Adaptar o Software às suas necessidades
- Redistribuir versões modificadas gratuitamente, nos termos da seção 4

### 2.4 CONTRIBUIÇÃO
- Submeter pull requests ao repositório oficial
- Reportar bugs e sugerir melhorias
- Participar de discussões sobre o projeto

---

## 3. RESTRIÇÕES

Você **NÃO** tem permissão para:

### 3.1 VENDA E COMERCIALIZAÇÃO
- Vender o Software, modificado ou não
- Cobrar qualquer valor pela distribuição
- Incluir o Software em pacotes pagos
- Oferecer versões "premium", "pro" ou "empresarial" mediante pagamento
- Monetizar através de doações obrigatórias para acesso

### 3.2 REDISTRIBUIÇÃO COMERCIAL
- Redistribuir o Software mediante compensação financeira
- Disponibilizar o Software mediante pagamento em marketplaces ou lojas de aplicativos
- Usar o Software como produto em modelos de negócio SaaS sem autorização
- Licenciar ou sublicenciar o Software em termos diferentes desta licença

### 3.3 MODIFICAÇÕES COMERCIAIS
- Criar e distribuir versões modificadas do Software para venda
- Desenvolver produtos derivados comerciais sem autorização expressa

### 3.4 REMOÇÃO DE ATRIBUIÇÕES
- Remover, ocultar ou modificar avisos de copyright
- Remover créditos ao autor original
- Apresentar o Software ou suas modificações como trabalho próprio original
- Alterar ou remover esta licença

### 3.5 USOS MALICIOSOS
- Usar o Software para atividades ilegais
- Distribuir versões que contenham malware, spyware ou código malicioso

---

## 4. REDISTRIBUIÇÃO GRATUITA

Redistribuição gratuita é **permitida** sob as seguintes condições:

### 4.1 REQUISITOS OBRIGATÓRIOS
Você deve:
- ✅ Incluir uma cópia completa desta licença
- ✅ Manter todos os avisos de copyright intactos
- ✅ Dar crédito claro ao projeto original (PawnPro-Core by NullSablex)
- ✅ Não apresentar como produto comercial ou proprietário
- ✅ Incluir link para o repositório oficial: https://github.com/NullSablex/PawnPro-Core

### 4.2 MODIFICAÇÕES REDISTRIBUÍDAS
Se redistribuir versões modificadas gratuitamente:
- Deve identificar claramente as modificações realizadas
- Deve incluir data das modificações
- Deve manter esta licença
- Não pode implicar endosso do autor original

---

## 5. CONTRIBUIÇÕES

### 5.1 ACEITAÇÃO DE CONTRIBUIÇÕES
Ao enviar contribuições (pull requests, issues, patches, sugestões) você concorda que:
- Suas contribuições serão licenciadas sob os mesmos termos desta licença
- Você concede ao autor original, de forma perpétua e irrevogável, o direito de relicenciar suas contribuições junto com o projeto, inclusive sob outros termos
- Você tem direito legal de conceder estas permissões
- O autor original mantém todos os direitos sobre o projeto como um todo

### 5.2 DIREITOS DO AUTOR
O autor original reserva-se o direito de:
- Aceitar ou rejeitar contribuições a seu critério
- Modificar contribuições aceitas
- Mudar a licença do projeto no futuro (não afetará versões já distribuídas)

---

## 6. LICENCIAMENTO COMERCIAL

Para obter permissões além das concedidas nesta licença, entre em contato:

- **GitHub:** https://github.com/NullSablex/PawnPro-Core
- **Issues:** https://github.com/NullSablex/PawnPro-Core/issues

---

## 7. MARCAS REGISTRADAS

"PawnPro", "PawnPro-Core" e logos associados são marcas do autor original. Esta licença não concede direito de usar marcas comerciais, exceto conforme necessário para identificar a origem do Software e dar crédito apropriado.

---

## 8. GARANTIAS E RESPONSABILIDADE

### 8.1 ISENÇÃO DE GARANTIAS
```
O SOFTWARE É FORNECIDO "COMO ESTÁ", SEM GARANTIAS DE QUALQUER TIPO,
EXPRESSAS OU IMPLÍCITAS, INCLUINDO, MAS NÃO SE LIMITANDO A:

- GARANTIAS DE COMERCIALIZAÇÃO
- ADEQUAÇÃO A UM PROPÓSITO ESPECÍFICO
- NÃO VIOLAÇÃO DE DIREITOS DE TERCEIROS
- PRECISÃO, CONFIABILIDADE OU DISPONIBILIDADE
```

### 8.2 LIMITAÇÃO DE RESPONSABILIDADE
```
EM NENHUMA CIRCUNSTÂNCIA O AUTOR SERÁ RESPONSÁVEL POR:

- DANOS DIRETOS, INDIRETOS, INCIDENTAIS OU CONSEQUENCIAIS
- PERDA DE DADOS, LUCROS OU OPORTUNIDADES DE NEGÓCIO
- QUALQUER DANO RESULTANTE DO USO OU IMPOSSIBILIDADE DE USO DO SOFTWARE
```

---

## 9. VIGÊNCIA E RESCISÃO

Esta licença é **automaticamente rescindida** se você violar qualquer termo. Após rescisão, você deve cessar todo uso do Software e destruir todas as cópias em sua posse.

---

## 10. DISPOSIÇÕES GERAIS

Esta licença é regida pelas leis do Brasil. Se qualquer cláusula for considerada inválida, as demais permanecem em vigor.

---

## 11. CLASSIFICAÇÃO DA LICENÇA

Esta é uma **licença de código-fonte disponível (source-available)**, não uma licença de código aberto (open source) conforme definida pela Open Source Initiative (OSI).

---

**Para questões sobre esta licença, abra uma issue em:**  
https://github.com/NullSablex/PawnPro-Core/issues

**Última atualização:** Setembro 2026  
**Versão da Licença:** 1.0
